...
<template>
  <div class="min-h-[100dvh] bg-paper">
    ...
    <main class="px-6 py-10 lg:px-10">
      <h1 class="font-display text-3xl text-foreground">Tus negocios, {{ auth.user?.name }}</h1>
      <p class="mt-1 text-muted-foreground">Gestiona cada negocio que tienes dado de alta.</p>

      <form class="mt-8 flex max-w-md items-end gap-3" @submit.prevent="onCreate">
